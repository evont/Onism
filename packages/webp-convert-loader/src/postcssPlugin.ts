...
async function convertToWebp(url, loaderContext) {
  try {
    ...
    const { info: { size }, data: oldData, hash } = await new Promise((resolve, reject) => {
    ...
    const { info: { size: webpSize }, data: webpData, hash: webpHash } = await new Promise((resolve, reject) => {
